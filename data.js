...
window.REVIEWERS = {
  pranav: { name: "Pranav Iyer", role: "Controller",              org: "Helios",   color: "#334155", accessory: "tie" },
  anton:  { name: "Anton Dam",   role: "Senior Accountant",       org: "Helios",   color: "#475569", accessory: "plain" },
  tara:   { name: "Tara Okoye",  role: "Chief Audit Executive",   org: "Helios",   color: "#1e293b", accessory: "tie" },
  naomi:  { name: "Naomi Wilkes",role: "Engagement Partner",      org: "Deloitte", color: "#1B5E20", accessory: "glasses" },
};
...
window.RC_047 = {
  ...
  defensibility_score: 96,
  status: "chain_complete",
  big4_visible: true,

  // ----- Control attributes (required for SOX 404 documentation) -----
  control_attributes: {
    control_owner: { name: "Pranav Iyer", role: "Controller", org: "Helios" },
    frequency: "Per-transaction (continuous)",
    nature: "Manual",
    type: "Preventive",
    key_control: true,
    automated_components: "NetSuite workflow rule WF-MJE-RR-047 enforces dual-approval gate; exception-report generation is automated. Approval judgment and exception disposition are human.",
  },
  ...
  // Procedures are grouped by test phase. SOX 404 testing distinguishes:
  //   - Test of Design (TOD): does the control as designed mitigate the risk?
  //   - Test of Operating Effectiveness (TOE): does the control actually operate as designed?
  testing_procedures: [
    { n: 1, phase: "TOD", text: "Obtain and inspect the control narrative and the NetSuite workflow configuration (rule WF-MJE-RR-047) to corroborate the threshold and dual-approval requirements as documented match what is enforced at the application layer.", tickmarks: ["✓"] },
    { n: 2, phase: "TOD", text: "Through inquiry and observation, walk through the end-to-end posting workflow for one in-scope manual journal entry to verify the workflow gates fire as designed and the exception report captures bypasses.", tickmarks: ["✓"] },
    { n: 3, phase: "TOE", text: "Obtain the population of all manual journal entries posted to in-scope revenue accounts during the testing period from NetSuite saved search NS-Saved-Search-MJE-ALL-047. Test IPE (Information Produced by Entity) — see IPE section.", tickmarks: ["PBC", "b"] },
    { n: 4, phase: "TOE", text: "Foot the population and reconcile total entries to general ledger activity in the in-scope revenue accounts for the period.", tickmarks: ["¶", "TB"] },
    { n: 5, phase: "TOE", text: "From the population, select a sample of 25 entries using stratified judgmental sampling: 100% of entries ≥ $1,000,000; judgmental selection across the remainder weighted toward period-end and unusual postings. Sample size and methodology pre-agreed with Deloitte engagement team in walkthrough on 2026-02-04 (memo MEMO-SS-RC-047).", tickmarks: ["§"] },
    { n: 6, phase: "TOE", text: "For each sampled entry, agree the approval evidence captured in NetSuite to the underlying support and verify the approver was independent of the preparer and at the appropriate authorization level for the entry amount.", tickmarks: ["✓", "c"] },
    { n: 7, phase: "TOE", text: "For entries ≥ $250,000, verify dual approval was obtained prior to posting; for entries < $250,000, verify single approval by an authorized Senior Accountant.", tickmarks: ["✓", "c"] },
    { n: 8, phase: "TOE", text: "Inspect the weekly exception report for the 13-week testing period and verify each flagged entry was dispositioned within 5 business days; agree any overrides to the override log and supporting rationale.", tickmarks: ["✓", "†"] },
    { n: 9, phase: "TOE", text: "Inquire of the Controller and inspect memo M-2026-Q1-MJE to corroborate that the $250,000 threshold was reassessed against current-period materiality and approved during the period.", tickmarks: ["‡"] },
  ],

  // Conclusion split across test phases (required for SOX testing).
  design_conclusion:
    "The control as designed addresses the identified risk through (a) a system-enforced workflow at the $250,000 threshold within NetSuite, (b) an independent reviewer requirement at the appropriate authorization level, and (c) weekly exception monitoring with documented disposition. Design effectively addresses the risk of unauthorized or inaccurate manual entries to revenue accounts.",
  operating_conclusion:
    "Based on inspection of 25 sampled entries from a population of 218, plus inspection of the weekly exception report for the 13-week period, plus inquiry and inspection of the threshold-review memo, the control operated effectively throughout Q1 2026. Two minor exceptions noted (see Exceptions section); neither indicates a control deficiency.",

  // IPE — Information Produced by Entity — required by AS 2201 / PCAOB.
  // When reports from the company's systems are used as audit evidence,
  // the auditor must verify the report's completeness and accuracy.
  ipe_testing: {
    items: [
      {
        source: "NS-Saved-Search-MJE-ALL-047 (population query)",
        completeness: "Saved-search definition inspected; query parameters agreed to control narrative scope (in-scope revenue accounts, manual-entry source flag, testing date range). Total row count reconciled to general-ledger manual-entry activity in revenue accounts for the period — no variance.",
        accuracy: "Five entries from the export randomly re-traced to NetSuite source records; field-level values (entry id, posting date, account, amount, preparer, approver) agreed without exception.",
      },
      {
        source: "NS-Saved-Search-MJE-EX-047 (weekly exception report)",
        completeness: "Saved-search definition inspected; flagging logic agreed to exception-report design memo. Spot-tested three weeks from the population query to confirm flagged entries appeared on the exception report — no completeness gaps identified.",
        accuracy: "All flagged entries in the testing period re-tested for the underlying flag condition (threshold-bypass, round-dollar, manual-offset pattern); each flag agreed to its trigger.",
      },
    ],
    conclusion: "IPE relied upon is complete and accurate for the testing period; appropriate to use as audit evidence.",
  },
  sample_selection: {
    population: 218,
    sample_size: 25,
...
window.RC_047_CHAIN = {
  ...
  nodes: [
    ...
    { id: "art_rc047_r1", label: "NS workflow screenshot", type: "artifact", parent: "proc_1" },
    { id: "art_rc047_walk", label: "Walkthrough memo", type: "artifact", parent: "proc_2" },
    { id: "art_rc047_q1", label: "Population query", type: "artifact", parent: "proc_3" },
    { id: "art_rc047_q1b", label: "Footing recompute", type: "artifact", parent: "proc_4" },
    { id: "art_rc047_s1", label: "Sample selection memo", type: "artifact", parent: "proc_5" },
    { id: "art_rc047_r1b", label: "Approval evidence export", type: "artifact", parent: "proc_6" },
    { id: "art_rc047_r1c", label: "Per-sample approval log", type: "artifact", parent: "proc_7" },
    { id: "art_rc047_q2", label: "Weekly exception report", type: "artifact", parent: "proc_8" },
    { id: "art_rc047_a1", label: "Threshold memo M-2026-Q1-MJE", type: "artifact", parent: "proc_9" },
    // sign-offs (edges)
...
window.FDE_POD = {
  jamie:  { name: "Jamie Okonkwo", role: "FDE Pod Lead", initials: "JO", color: "#b45309", accessory: "tie",     org: "Optro" },
  priya:  { name: "Priya Raman",   role: "Senior FDE",   initials: "PR", color: "#1e3a8a", accessory: "glasses", org: "Optro" },
  marcus: { name: "Marcus Chen",   role: "FDE Analyst",  initials: "MC", color: "#6d28d9", accessory: "plain",   org: "Optro" },
};
